module Isomorfeus
  module Record
    class Collection < Array
      ...
      # @param owning_record [Isomorfeus::Record] optional base record this collection belongs to
      # @param relation_name [String] optional base record relation name this collection represents
      def initialize(collection = [], owning_record = nil, relation_name = nil)
        @owning_record = owning_record
      ...
      def <<(other_record, call_promise = true)
        if @owning_record && @relation_name
          Isomorfeus::DataAccess.register_used_store_path(:record, @owning_record.class.model_name, :instances, @owning_record.id, :relations,
                                                          @relation_name)
        end
      ...
      def _promise_link(other_record)
        if @owning_record && @relation_name
          Isomorfeus.store.dispatch(type: 'RECORD_ADD_TO_RELATION', model: @owning_record.class.model_name, id: @owning_record.id,
                                    object_id: @owning_record.object_id, relation: @relation_name, other_model: other_record.class.model_name,
                                    other_id: other_record.id, other_object_id: other_record.object_id)
          Isomorfeus::DataAccess.promise_fetch(:record, @owning_record.class.model_name, :instances, @owning_record.id, :add_to_relation,
      ...
      def _promise_unlink(other_record)
        if @owning_record && @relation_name
          Isomorfeus.store.dispatch(type: 'RECORD_REMOVE_FROM_RELATION', model: @owning_record.class.model_name, id: @owning_record.id,
                                    object_id: @owning_record.object_id, relation: @relation_name, other_model: other_record.class.model_name,
                                    other_id: other_record.id, other_object_id: other_record.object_id)
          Isomorfeus::DataAccess.promise_fetch(:record, @owning_record.class.model_name, :instances, @owning_record.id, :remove_from_relation,
                                       @relation_name, other_record.class.model_name, :instances, other_record.id)
        else
          Promise.new.resolve(self)
        end
