    ...
    class ClassStoreProxy
      ...
      def method_missing(key, *args, &block)
        if `key.endsWith('=')`
          ...
          if @native_component_instance.JS[:state].JS[:__component_class_state].JS[@component_name].JS[key]
            return @native_component_instance.JS[:state].JS[:__component_class_state].JS[@component_name].JS[key]
          end
