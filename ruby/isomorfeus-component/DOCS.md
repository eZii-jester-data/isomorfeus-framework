# Isomorfeus Components

## Components DSL Overview

Isomorfeus **Components** are implemented in the isomorfeus-component and isomorfeus-react Gems.

Isomorfeus Component DSL (Domain Specific Language) is a set of class and instance methods that are used to describe your React components.

The DSL has the following major areas:  

+ The `Isomorfeus::Component` class and the equivalent `Isomorfeus::Component::Mixin` mixin
...
### Isomorfeus::Component

Isomorfeus Components classes either include `Isomorfeus::Component::Mixin` or are subclasses of `Isomorfeus::Component`.  

```ruby
class Component < Isomorfeus::Component
end
...
```ruby
class AnotherComponent < Isomorfeus::Component
  render do
...
```ruby
class WelcomeUser < Isomorfeus::Component
  param: id
...
class SayHello < Isomorfeus::Component
...
In Isomorfeus, there are two exceptions to this rule:
...
```ruby
class Likes < Isomorfeus::Component
...
>**Note on coding style:** In the Isomorfeus documentation and tutorials we use uppercase HTML elements like `DIV` and `BUTTON` as we believe this makes for greater readability in the code; specifically with code highlighting. If you do not like this you can use lowercase `div` and `button` instead.
...
<pre>
class LikeButton < Isomorfeus::Component
...
```ruby
class Avatar < Isomorfeus::Component
  param :user_name
  render(DIV) do
...
class ProfilePic < Isomorfeus::Component
  param :user_name
  render do
...
class ProfileLink < Isomorfeus::Component
  param :user_name
  render do
...
```ruby
# WRONG!
class ListItemWrapper < Isomorfeus::Component
  param :data
  render do
...
```ruby
class ManyParams < Isomorfeus::Component
...
```ruby
class CheckLink < Isomorfeus::Component
...
Ruby has a rich set of mechanisms enabling code reuse, and Isomorfeus is intended to be a team player in your Ruby application.  Components can be subclassed, and they can include (or mixin) other modules.  You can also create a component by including `Isomorfeus::Component::Mixin` which allows a class to inherit from some other non-react class, and then mixin the React DSL.
...
class TickTock < Isomorfeus::Component
  ...
  render(DIV) do
    "Isomorfeus has been running for #{state.seconds} seconds".para
  end
end
</pre></div>
...
```ruby
class AComponent < Isomorfeus::Component
  before_mount do
...
```ruby
class Hello < Isomorfeus::Component
...
```ruby
class Alarm < Isomorfeus::Component
  param :at, type: Time
...
<pre>
class IndentEachLine < Isomorfeus::Component
...
class Indenter < Isomorfeus::Component
  render(DIV) do
...
For reasons described below Isomorfeus provides a render callback to simplify defining the render method:
...
Normally Isomorfeus will only update a component if some state variable or param has changed.  To override this behavior you can redefine the `should_component_update?` instance method.  For example, assume that we have a state called `funky` that for whatever reason, we
...
```ruby
class RerenderMore < Isomorfeus::Component
...
Why would this happen?  Most likely there is integration between new Isomorfeus Components and other data structures being maintained outside of Isomorfeus, and so we have to do some explicit comparisons to detect the state change.

Note that `should_component_update?` is not called for the initial render or when `force_update!` is used.

> Note to react.js readers.  Essentially Isomorfeus assumes components are "well behaved" in the sense that all state changes
...
```ruby
class YouSaid < Isomorfeus::Component
...
Here is a Isomorfeus version of this [w3schools.com](https://www.w3schools.com/html/html5_draganddrop.asp) example:
...
While it is quite possible to develop large applications purely in Isomorfeus Components with a ruby back end like rails, you may eventually find you want to use some pre-existing React Javascript library.   Or you may be working with an existing React-JS application, and want to just start adding some Isomorfeus Components.

Either way you are going to need to import Javascript components into the Isomorfeus namespace. Isomorfeus provides both manual and automatic mechanisms to do this depending on the level of control you need.

#### Importing Components

Lets say you have an existing React Component written in javascript that you would like to access from Isomorfeus.  
...
Assuming that this component is loaded some place in your assets, you can then access this from Isomorfeus by creating a wrapper Component:
...
class MyBigApp < Isomorfeus::Component
  render(DIV) do
    # SayHello will now act like any other Isomorfeus component
    SayHello name: 'Matz'
  end
end
```
...
Many React components come in libraries.  The `ReactBootstrap` library is one example.  You can import the whole library at once using the `React::NativeLibrary` class.  Assuming that you have initialized `ReactBootstrap` elsewhere, this is how you would bring it into Isomorfeus.
...
```ruby
class Show < Isomorfeus::Component
...
If you use a lot of libraries and are using a Javascript tool chain with Webpack, having to import the libraries in both Isomorfeus and Webpack is redundant and just hard work.

Instead you can opt-in for *auto importing* Javascript components into Isomorfeus as you need them.  Simply `require isomorfeus-react/auto-import` immediately after you `require isomorfeus-react`.  
...
*How it works:  The first time Ruby hits a native library or component name, the constant value will not be defined.  This will trigger a lookup in the javascript name space for the matching component or library name.  This will generate either a new subclass of Isomorfeus::Component or React::NativeLibrary that imports the javascript object, and no further lookups will be needed.*

#### Including React Source  

If you are in the business of importing components with a tool like Webpack, then you will need to let Webpack (or whatever dependency manager you are using) take care of including the React source code.  Just make sure that you are *not* including it on the ruby side of things. Isomorfeus is currently tested with React versions 13, 14, and 15, so its not sensitive to the version you use.
...
+ In the rails isomorfeus initializer you can say:

 ```ruby
 Isomorfeus.configuration do |config|
   config.prerendering = :on # :off by default
 end
 ```

+ In a route you can override the config setting by setting a default for isomorfeus_prerendering:

```ruby
get '/some_page', to: 'isomorfeus#some_page', defaults: {isomorfeus_prerendering: :off} # or :on
```
...
+ You can override the route, and config setting using the isomorfeus-prerendering query param:

```html
http://localhost:3000/my_hyper_app/some_page?isomorfeus-prerendering=off
```

This is useful for development and testing

NOTE: in the route you say isomorfeus_prererendering but in the query string its isomorfeus-prerendering (underscore vs. dash). This is because of rails security protection when using defaults.

### Further Reading

**Note:** The Isomorfeus gems have recently been renamed. The links below will take you to the correct Github projects but you might find the name of the project does not quite match the name of the gem on this page. Isomorfeus Components were previously known as HyperReact or Reactrb.

#### Other Isomorfeus tutorials and examples
+ [Isomorfeus Tutorials](http://ruby-isomorfeus.io/tutorials/)

#### React under the covers

Isomorfeus Components and friends are in most cases simple DSL Ruby wrappers to the underlying native JavaScript libraries and React Components. It is really important to have a solid grip on how these technologies work to complement your understanding of Isomorfeus. Most searches for help on Google will take you to examples written in JSX or ES6 JavaScript but you will learn over time to translate this to Isomorfeus equivalents. To make headway with Isomorfeus you do need a solid understanding of the underlying philosophy of React and its component based architecture. The 'Thinking in React' tutorial below is an excellent place to start.
...
Isomorfeus Components are a DSL wrapper of React which uses Opal to compile Ruby code to ES5 native JavaScript. If you have not used Opal before then you should at a minimum read the excellent guides as they will teach you enough Opal to get you started with Isomorfeus.
