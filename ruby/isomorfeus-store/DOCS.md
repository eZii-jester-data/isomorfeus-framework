...
```ruby
class UserStore < Isomorfeus::Store
...
+ `Isomorfeus::Store::Mixin` can be mixed in to any class to turn it into a Flux Store.
+ You can also create Stores by subclassing `Isomorfeus::Store`.
...
`state.items` reads the current value of the state named `items`.  Isomorfeus tracks all reads of state, and mutating those states will trigger a re-render of any Components depending on the current value.

`mutate.items` returns the current value of the state named `items`, but also tells Isomorfeus that the value is changing, and that any Components depending on the current value will have to be re-rendered.
...
```ruby
class Cart < Isomorfeus::Store
  state items: Hash.new { |h, k| h[k] = 0 }, scope: :class, reader: true
end
```

This *declares* the `items` state as a class state variable, will initialize it with the hash on `Isomorfeus::Boot`, and provides a reader method.
...
```ruby
class MyStore < Isomorfeus::Store
...
class Cart < Isomorfeus::Store
  ...
  # initialize the hash by receiving the system Isomorfeus::Application::Boot or Empty dispatches

  receives Isomorfeus::Application::Boot, Empty do
    mutate.items(Hash.new { |h, k| h[k] = 0 })
  end
...
Isomorfeus is less opinionated.  You may also add mutator methods to your class.  Our recommendation is that you append an exclamation (!) to methods that mutate state.
