    ...
    class << self
      ...
      def initialize_values(klass, name, opts)
        ...
        if initializer && opts[:block]
          klass.receives(Isomorfeus::Application::Boot, initializer) do
            klass.mutate.__send__(:"#{name}", opts[:block].call)
          end
        elsif initializer
          klass.receives(Isomorfeus::Application::Boot, initializer)
        elsif opts[:block]
          klass.receives(Isomorfeus::Application::Boot) do
            klass.mutate.__send__(:"#{name}", opts[:block].call)
          end
        end
      end
