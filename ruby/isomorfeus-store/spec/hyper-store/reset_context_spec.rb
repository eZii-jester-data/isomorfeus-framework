  ...
  it "does not reset any predefined boot receivers", js: true do
    on_client do
      class Store < Isomorfeus::Store
        ...
        receives Isomorfeus::Application::Boot, :booted
      end
      puts ">>resetting"
      Isomorfeus::Context.reset!(reboot = nil)
      class Store < Isomorfeus::Store
        receives Isomorfeus::Application::Boot, :another_receiver
      end
    end
    evaluate_ruby("puts '>>booting'") # do a separate evaluation to get initial boot completed
    evaluate_ruby do
      puts '>>resetting again'
      Isomorfeus::Context.reset!(nil)
      puts '>>booting again'
      Isomorfeus::Application::Boot.run
    end
