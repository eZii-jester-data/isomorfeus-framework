module ExampleMaterial
  class AMaterialComponent < LucidMaterial::Component::Base
    ...
    render do
      SPAN(on_click: props.on_click, class_name: classes.root) { 'Click for orange alert! Props: ' }
      SPAN { props.text }
      SPAN(on_click: :change_state, class_name: classes.root) { ", state is: #{state.some_bool} (Click!)" }
      SPAN(on_click: :change_store, class_name: classes.root) { ", store is: #{store.a_bool} (Click!)" }
      SPAN(on_click: :change_class_store, class_name: classes.root) { ", class_store is: #{class_store.b_bool} (Click!)" }
      SPAN(on_click: :change_app_store, class_name: classes.root) { ", app_store is: #{app_store.c_bool} (Click!)" }
      SPAN { ', Children: '  }
      SPAN { props.children }
      SPAN { '| '}
    end
